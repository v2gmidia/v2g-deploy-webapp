-- ============================================================
-- 0023 — as CORES DA MARCA entram na lista branca do cliente
--
-- NAO APLICADA. Escrita em 21/09/2026; quem roda `db:migrate` e uma pessoa.
--
-- ------------------------------------------------------------
-- O QUE MUDA: tres linhas no array de `identidade_visual`. Nada mais.
--
-- A funcao e reescrita INTEIRA, e nao remendada, pelo motivo que a 0016
-- ja escreveu: nao existe ALTER para um literal de array dentro de um
-- corpo plpgsql, e reescrever a mao o que ja estava certo e como se perde
-- uma checagem sem ninguem ver. O `git diff` entre a 0016 e este arquivo
-- mostra exatamente o bloco do `identidade_visual` e este cabecalho.
--
-- ------------------------------------------------------------
-- POR QUE ELA EXISTE
--
-- O onboarding v3 tira as 2 ou 3 cores principais da logo no navegador e
-- PERGUNTA se sao as cores da empresa. A resposta precisa de onde cair.
--
-- AS COLUNAS JA EXISTEM. `identidade_visual.cor_primaria`,
-- `cor_secundaria` e `cor_destaque` estao la desde a `0010_perfil_empresa`.
-- Nao falta coluna — falta a PORTA: a `confirmar_campo_do_cliente` e a
-- unica que grava valor e procedencia na mesma transacao, e a lista branca
-- dela nao aceita as tres.
--
-- ------------------------------------------------------------
-- ISTO REVERTE UMA DECISAO DA 0015, E O MOTIVO DELA CAIU
--
-- A 0015 deixou cor e fonte de fora com esta justificativa, que esta
-- escrita no arquivo dela:
--
--   "cor e fonte ficam de fora pelo mesmo motivo do catalogo: sao lidas
--    pela geracao de criativo como codigo de cor e nome de fonte"
--
-- MEDIDO EM 21/09/2026, contra o instantaneo do backend: o agente
-- `gerar_criativo_visual` recebe `nicho`, `angulo_origem`, `texto_overlay`
-- e `formatos` — e NENHUM campo de cor. O `prompt.md` dele manda a cor vir
-- das FOTOS ("iluminacao e cor coerentes com as fotos recebidas"). O unico
-- lugar do backend que fala em paleta e o `varrer_site`, que a extrai do
-- HTML do site e a guarda na saida dele, sem devolver para a identidade.
--
-- Ou seja: hoje ninguem le essas tres colunas. A justificativa de 2015
-- descrevia uma leitura que nao acontece.
--
-- Isso NAO quer dizer que a decisao estava errada quando foi tomada — quer
-- dizer que ela pode ser revista agora, e que revisar custa esta migration.
-- O que continua de fora: `fonte_titulo` e `fonte_corpo`. Nome de fonte
-- digitado por cliente e caminho para um criativo que nao renderiza, e
-- ninguem pediu isso.
--
-- ------------------------------------------------------------
-- O QUE ISTO NAO RESOLVE
--
-- Gravar nao e usar. Depois desta migration as cores tem onde cair e o
-- cliente pode corrigi-las pela /meu-negocio — e o criativo continua sem
-- le-las. O que falta do lado do backend esta em DUVIDAS.md, ONB-14.
-- ============================================================
...
begin
  ...
  v_permitidos := case p_tabela
    ...
    when 'identidade_visual' then array[
      ...
      -- Acrescentadas pela 0023. A justificativa da 0015 para deixá-las
      -- fora era que a geração de criativo as lia como código de cor —
      -- medido em 21/09/2026, ela não lê: a entrada do agente
      -- `gerar_criativo_visual` não tem campo de cor nenhum.
      --
      -- Elas entram porque o onboarding as tira da LOGO e pergunta ao dono
      -- se são as dele. Resposta que o dono deu precisa de onde cair.
      'cor_primaria', 'cor_secundaria', 'cor_destaque'
      -- `fonte_titulo` e `fonte_corpo` continuam FORA: nome de fonte
      -- digitado por cliente é caminho para criativo que não renderiza, e
      -- ninguém pediu isso.
    ]
  end;
...
comment on function public.confirmar_campo_do_cliente(uuid, uuid, text, text, jsonb) is
  'Unica porta que produz procedencia `confirmado`. Desde a 0023 aceita as tres cores da marca. Deriva o ato (confirmou/corrigiu/preencheu) do estado real da coluna, grava valor e procedencia na mesma transacao, e limpa geo_key quando a cidade muda. So service_role.';
