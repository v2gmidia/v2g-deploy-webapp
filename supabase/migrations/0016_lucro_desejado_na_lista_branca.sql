-- ============================================================
-- 0016 — `target_profit_per_customer` entra na lista branca do cliente
--
-- UMA LINHA DE DIFERENÇA para a 0015, e ela é obrigatória: a lista branca
-- da `confirmar_campo_do_cliente` foi escrita contra o catálogo de
-- `lib/agentes/campos.ts` como ele era naquele dia. O bloco 2 do
-- onboarding (docs/onboarding-expandido.md §3.2) acrescentou
-- `target_profit_per_customer` ao catálogo, e a própria 0015 manda:
-- "Campo novo no catálogo entra aqui também, e por migration."
--
-- Sem isto a conta 3 grava valor sem procedência — ou não grava.
--
-- A FUNÇÃO É REESCRITA INTEIRA, e não remendada. Não existe ALTER para um
-- literal de array dentro de um corpo plpgsql, e reescrever à mão o que já
-- estava certo é como se perde uma checagem sem ninguém ver. O corpo abaixo
-- é cópia mecânica da 0015 com a linha nova e a contagem do comentário
-- corrigida — o `git diff` entre os dois arquivos mostra exatamente isso.
--
-- Os grants são repetidos por serem baratos e por `create or replace` não
-- ser o único caminho até aqui. `public`, `anon` E `authenticated`.
-- ============================================================
